...
\begin{itemize}
  ...
  \item \good{\textbf{Seed 0}}: full curve, $N{=}20 \to \mathbf{47.5\,\%}$ \textemdash{} beats every other method by 7\textendash 22\,pp.
  \item \textbf{N=10 robustness stress test} (4 seeds, paired vs random): quota wins \good{3 of 4} seeds, \good{mean $+7.0$\,pp}. Seed 1 ($-9.5$) is the lone outlier.
  \item \textbf{Current read}: per-task coverage is a \good{real} effect at $N{=}10$, not a one-seed fluke. Full $N{=}20$ curves now justified to confirm it carries to the full budget.
\end{itemize}
\end{frame}
