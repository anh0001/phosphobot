...
\begin{itemize}
  ...
  \item \good{\textbf{Seed 0}}: full curve, $N{=}20 \to \mathbf{47.5\,\%}$ \textemdash{} beats every other method by 7\textendash 22\,pp. Looked like the win.
  \item \bad{\textbf{Seed 1}}: $N{=}10 \to 21.5\,\%$, which is \bad{9.5\,pp \emph{under} random} at the same budget (OOM'd before $N{=}20$).
  \item \textbf{Current read}: the 47.5\,\% is likely single-seed lucky task coverage, not a robust effect. Headline \bad{not yet defensible}.
\end{itemize}
\end{frame}
...
\toprule
$N$ & rand s0 & rand s1 & \muted{conf s0} & \muted{conf s1} & ent s0 & disp s0 & d+q s0 & disp s1 & d+q s1 \\
\midrule
5  & 9.0  & 10.5 & \muted{9.0}  & \muted{10.5} & 9.0  & 9.0          & 9.0          & 10.5 & 10.5 \\
10 & 22.5 & 31.0 & \muted{21.0} & \muted{16.0} & 15.5 & 29.0         & 25.0         & \bad{16.5} & \bad{21.5} \\
15 & 33.0 & 32.5 & \muted{24.5} & \muted{18.5} & 25.5 & \bad{25.0}   & 29.5         & \bad{OOM} & \bad{OOM} \\
20 & 35.0 & 40.5 & \muted{28.0} & \muted{25.0} & 29.0 & \bad{OOM}    & \good{\textbf{47.5}} & \bad{OOM} & \bad{OOM} \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  ...
  \item \good{d+q s0 $=47.5$ at $N{=}20$} was the candidate win\textemdash{} but \bad{d+q s1 is under random at $N{=}10$ (21.5 vs 31.0)}. One seed up, one seed down.
  \item \muted{Grayed conformal columns}: implementation broken; kept for reproducibility only.
  \item \bad{OOM}: GPU co-tenant returned mid-train; partial curve only.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item 5 methods on libero\_spatial; conformal bug found (rank-invariant violated).
  \item Pure \texttt{dispersion} regresses at $N{=}15$ \good{reproducibly} (both seeds, both runs).
  \item \texttt{dispersion\_quota} s0 hit 47.5\,\% at $N{=}20$, but s1 is under random at $N{=}10$.
\end{itemize}

\textbf{Open question}
\begin{itemize}
  \item Is \texttt{dispersion\_quota} a \emph{robust} low-budget learner, or was 47.5\,\% a one-seed fluke?
\end{itemize}

\textbf{Recurring blocker}
\begin{itemize}
  \item A co-tenant (HMDB51 ConvGRU) keeps grabbing both RTX\,6000s mid-run and OOM-killing the $N{=}15$\textendash $20$ rounds. 3 cells lost this way so far.
\end{itemize}
\end{frame}

% --------------------------------------------------------------------
\begin{frame}{Next experiment (codex-recommended): cheap N=10 stress test}
\textbf{Plan}: skip full curves. Run \texttt{random} and \texttt{dispersion\_quota} at \emph{fixed} $N{=}10$ (2 rounds, $\sim$2.5\,h/cell) on \textbf{two new seeds} (s2, s3), paired demos + paired eval. Pure \texttt{dispersion} dropped \textemdash{} already failed the robustness test.

\vspace{0.6em}
\textbf{Decision rule} (set before looking):
\begin{itemize}
  \item quota \bad{loses} to random on both s2, s3 $\Rightarrow$ drop the method-win claim.
  \item quota \good{ties/wins} on both $\Rightarrow$ s1 was the outlier; run full $N{=}20$ curves for quota only.
  \item \textbf{split} $\Rightarrow$ no method win; pivot to the methodological paper.
\end{itemize}

\vspace{0.6em}
\textbf{Caveat}: $N{=}10$ tests \emph{robustness}, not the $N{=}20$ headline (s0 was mediocre early, spiked late \textemdash{} a possible coverage-threshold effect $N{=}10$ can't rule out).
\end{frame}
